...
export const BatchTransfer = () => {
    ...
    const [tokenAddress, setTokenAddress] = useState('0xDF617aA28bbdC3F1004291e1dEC24c617A4AE3aD')
    const [isLoading, setIsLoading] = useState(false)
    const [selectedToken, setSelectedToken] = useState(tokensList[2]) // MEXA selected by default
  ...
  useEffect(() => {
    ...
    const handleContactAmountChange = (contactId: string, amount: string) => {
	...
	if (tab === 'contacts' && recipients.length > 0 && selectedContacts.length === 0) {
	    if (window.confirm('¿Desea mantener los destinatarios actuales del CSV?')) {
		// Keep recipients from CSV
	    } else {
		// Clear recipients when switching tabs
		setRecipients([]);
		setAmounts([]);
	    }
	}
	...
	const newRecipients = selectedContacts.map(contact => contact.walletAddress)
	
	const newAmounts = selectedContacts.map(() => "0")
  ...
  const handleTokenChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const selected = tokensList.find(token => token.address === event.target.value) || tokensList[2]
  ...
  if (chainId === SEPOLIA || chainId === ARBITRUM) {
	...
	<div className="bg-sectionBackground rounded-xl p-5 shadow-sm">
	    ...
	    {/* Token Selection (Common for both tabs) */}
	    <div className="mb-5">
		...
		<>
		    ...
		    {/* Selected Contacts List with Amount Inputs */}
			...
			<div className="mb-5">
				...
				{selectedContacts.map((contact, index) => (
				    <div key={contact.id} className="flex items-center justify-between bg-gray-50 rounded-xl p-3">
					<div>
					...
					<div className="w-24">
					    <input
						type="text"
						      className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-right"
						      placeholder="0.0"
						      value={amounts[index] || ""}
						      onChange={(e) => handleContactAmountChange(contact.id, e.target.value)}
					    />
					</div>
				    </div>
				))}
		...
		<div className="mb-5 bg-gray-50 rounded-xl p-4 border border-gray-200">
			...
			<span className="text-sm font-bold">
			    {amounts.reduce((acc, amt) => acc + parseFloat(amt || "0"), 0).toFixed(4)} {selectedToken.symbol}
			</span>
		    </div>
		</div>
	    )}
